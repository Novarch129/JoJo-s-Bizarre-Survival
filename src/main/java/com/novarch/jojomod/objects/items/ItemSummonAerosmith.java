...
public class ItemSummonAerosmith extends Item
{
	public ItemSummonAerosmith(Properties properties)
	{
		super(properties);
	}
	...
	{
		return Rarity.create("aerosmith", TextFormatting.LIGHT_PURPLE);
	}
		...
		if (!world.isRemote && entity instanceof PlayerEntity)
		{
			...
			JojoProvider.getLazy(player).ifPresent(props -> {
				if (props.getStandID() == 0)
				{
					...
					props.setStandID(JojoLibs.StandID.aerosmith);
					props.setStandOn(true);
					final EntityStandBase theStand = JojoLibs.getStand(JojoLibs.StandID.aerosmith, world);
					if (theStand != null)
					{
